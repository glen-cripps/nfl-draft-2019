...
combine.atomic = subset(combine.atomic,pos %in% c('OLB','DE','EDGE'))  # stick to RB's
View(combine.atomic)
...
plot( training_data$first_round_pick ~ training_data$defense.sacks)
glm(training_data$first_round_pick ~ training_data$defense.sacks , family=binomial(link='logit'))

#curve(predict(training_data,data.frame(training_data$first_round_pick=x),type="resp"),add=TRUE) 

abline(0.38706, 0.009368  )
