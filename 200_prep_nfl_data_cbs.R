...
college.position = subset(college.stats, college.stats$section == "defense")
#View(college.stats)
...
combine.atomic = subset(combine.atomic,pos=='CB')  # stick to RB's
#View(combine.atomic)
...
college.flat = spread(college.atomic, fullstat, value, fill=0)   
View(college.flat)
...
dq_list = rowSums(is.na(all_position)) # gotta go back and check if this is working as I expected.. dont think it is
# ya dummy fix that tomorrow 
...
imputed_data$pick_round = trunc((imputed_data$pick-1)/32)+1
imputed_data$first_round_pick = ifelse(imputed_data$pick_round==1,1,0)
...
training_data = select(imputed_data[imputed_data$year<=2017,], pick, first_round_pick,
                       year 
...
prediction_data = select(imputed_data[imputed_data$year==2018,], url, pick, first_round_pick,
                       year 
                       , weight , forty , vertical , bench , broad , threecone , shuttle , height_inches
                       , defense.games, defense.int, defense.int.td, defense.sacks, defense.seasons, defense.tackles
)
...
prediction_data$predicted_first_round_prob = predict(logistic_model, newdata = prediction_data, type="response")
...
plot(training_data$pick ~ training_data$forty)
lm(training_data$pick ~ training_data$forty  )
abline(lm(training_data$pick ~ training_data$forty  ) )

plot(training_data$pick ~ training_data$defense.int )
lm(training_data$pick ~ training_data$defense.int )
abline(lm(training_data$pick ~ training_data$defense.int ))

plot(training_data$pick ~ training_data$height_inches )
lm(training_data$pick ~ training_data$height_inches )
abline(lm(training_data$pick ~ training_data$height_inches ))

plot(training_data$pick ~ training_data$weight )
lm(training_data$pick ~ training_data$weight )
abline(lm(training_data$pick ~ training_data$weight ))

library("plot3D")
scatter3D(training_data$defense.int, training_data$forty, training_data$pick )
lm(training_data$pick ~ training_data$defense.int + training_data$forty + training_data$height_inches + training_data$weight)
abline(muh_line  )
-2136+ (-6.757)*20 +(519.790)*4.4
-2042+185
