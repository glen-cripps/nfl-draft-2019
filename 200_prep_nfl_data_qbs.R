...
View(college.stats[college.stats$url=="https://www.sports-reference.com/cfb/players/baker-mayfield-1.html",])
...
college.position = subset(college.stats, college.stats$section == "passing")
...
combine.atomic = subset(combine.atomic,pos %in% c('QB'))  # stick to RB's
View(combine.atomic)
...
training_data = select(imputed_data[imputed_data$year<=2017,], pick, first_round_pick,
                       ...
                       ,passing.attempts           
                       ,passing.completions , passing.games                 
                       ,passing.pass.ints , passing.pass.tds             
                       ,passing.pass.yards , passing.seasons 
                       )
...
linear_model = lm(first_round_pick ~ 
                    year 
                  + weight + forty + vertical + bench + broad + threecone + shuttle + height_inches
                  + passing.attempts           
                  + passing.completions + passing.games                 
...
plot(training_data$passing.pass.tds ~ training_data$first_round_pick)
lm(training_data$passing.pass.tds ~ training_data$first_round_pick )
abline(3.6, 0.0005221  )
...
prediction_data$predicted_first_round_prob = predict(logistic_model, newdata = prediction_data, type="response")
#csv_affairs_test$prob <-                     predict(csv_model,      newdata=csv_affairs_test, type="response")
